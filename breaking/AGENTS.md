...
**Timeline = the lead story's own chronology ("घटनाक्रम").** There is ONE mode: the timeline is
always the developing lead story's own arc, built from the **enriched lead cluster's own articles**
(`_cluster_points(top)`), so the **headline and the timeline always describe the same story** — they
are generated by Groq from the same coverage. Older same-story points from the 30-day archive are
folded in only when they share ≥`ENRICH_MIN_SHARED` of the lead's distinctive title terms, so a
different activity of the same body never leaks in. A genuine one-off is padded from its own
`key_facts`/`what_next` (`ensure_timeline_depth`). The list is **descending (newest first)** and
reveals on scroll (`IntersectionObserver`). The bottom **स्रोत (Source)** cards come from the
timeline arc's own events (`arc_sources` + expanded `HINDI_SOURCE`), so the source links always match
the timeline (they fall back to the lead cluster's own sources only if the arc yields none). Target
output is the golden [`breaking-benchmark.md`](breaking-benchmark.md) (machine-readable copy in
[`breaking-cases.json`](breaking-cases.json)).
